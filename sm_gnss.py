...
class SatelliteShadowMatching:
    ...
    def process_epoch(self, satellite_data, initial_position, buildings,building_triangles, model,cov_gnss):
        """
        ...
        if(len(best_points) == len(scores)):
        # # Calculate final position (average of best points)
            final_e = np.mean([p[0] for p in best_points])
            final_n = 0        #np.mean([p[1] for p in best_points])
            final_u = np.mean([p[2] for p in best_points])
            final_x, final_y, final_z = self.transformer.convert_enu_to_ecef(final_e, final_n, final_u,
            ...
            print('difffifffff')
            fused_enu = covariance_fusion(ref_lat, ref_lon, ref_alt,cov_gnss,scores)
            final_x, final_y, final_z = self.transformer.convert_enu_to_ecef(fused_enu[0], 0, fused_enu[2], ref_lon, ref_lat, ref_alt)
            return {
                'X': final_x,
                'Y': final_y,
                'Z': final_z,
        ...
        # # # Calculate final position (average of best points)
        #     final_e = np.mean([p[0] for p in best_points])
        #     final_n = np.mean([p[1] for p in best_points])
        #     final_u = np.mean([p[2] for p in best_points])
        #     final_x, final_y, final_z = self.transformer.convert_enu_to_ecef(final_e, final_n, final_u,
        #                                                                  ref_lon, ref_lat, ref_alt)
        #     return {
    ...
    def run(self, satellite_file, position_file, building_file, output_file):
        ...
        satellite_data,cov_data_gnss = self.data_parser.parse_satellite_data_cov(satellite_file)
        ...
        kml.save("F:\\GNSSdata\Android GNSS data\\2025-04-15\\GNSS_SM\\gnss_sm_preproc(P3)_yanshou.kml")
...
if __name__ == "__main__":
    ...
    sm_results = matcher.run(
        "F:\\GNSSdata\Android GNSS data\\2025-04-15\\GNSS_SM\\satpos_cov_new(P3).txt",
       #"F:\\GNSSdata\Android GNSS data\\2025-04-15\\GNSS_SM\\spp_cov(P1).pos",
        "F:\\GNSSdata\Android GNSS data\\2025-04-15\\ALL\\spp(P3)_zscore.csv",
        "files\\buildings.txt",
        "F:\\GNSSdata\Android GNSS data\\2025-04-15\\GNSS_SM\\gnss_sm_preproc(P3)_yanshou.csv"
    )