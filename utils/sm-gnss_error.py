...
input_csv_2 = "F:\\GNSSdata\\Android GNSS data\\2025-04-15\\GNSS_SM\\gnss_sm_preproc(P3).csv"
...
input_pos = "F:\\GNSSdata\\Android GNSS data\\2025-04-15\\ALL\\spp(p3).pos"
output_csv = "F:\\GNSSdata\\Android GNSS data\\2025-04-15\\GNSS_SM\\gnss_sm_preproc(P3)_error_yanshou1.csv"
figure_title = 'P3'
...
#-----------------------------------读取 input_csv_2的 CSV 数据---------------------------------------
# 读取 shadow matching 的 CSV 数据
csv_df2 = pd.read_csv(input_csv_2)  # 修改路径
csv_coords = csv_df2[['X', 'Y', 'Z']].values
csv_df2['Timestamp'] = pd.to_datetime(csv_df2['Epoch'])
...
# 处理 CSV 坐标 → 经纬度
csv_lons, csv_lats, _ = zip(*[ecef_to_wgs84.transform(x, y, z) for x, y, z in csv_coords])
csv_df2['Latitude'] = csv_lats
csv_df2['Longitude'] = csv_lons
csv_df2['easting'], csv_df2['northing'] = zip(*[wgs84_to_utm.transform(lon, lat) for lon, lat in zip(csv_df2['Longitude'], csv_df2['Latitude'])])

true_easting, true_northing = wgs84_to_utm.transform(true_lon, true_lat)
csv_df2['east_error'] = (csv_df2['easting'] - true_easting).abs()
csv_df2['north_error'] = (csv_df2['northing'] - true_northing).abs()
csv_df2['horizontal_error'] = np.sqrt(csv_df2['east_error']**2 + csv_df2['north_error']**2)
...
combined_df = pd.concat([csv_df0[['Timestamp', 'east_error', 'north_error', 'horizontal_error', 'Source']],
                        ...
                        csv_df2[['Timestamp', 'east_error', 'north_error', 'horizontal_error', 'Source']],
...
csv_stats2 = compute_error_stats(csv_df2)
...
labels = ['XGB-SM','GNSS-XGB-SM','SM','SPP']
...
plt.ylabel('Cross-street Error(m)', fontsize=20)
