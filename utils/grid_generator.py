...
class GridGenerator:
    ...
    def generate_search_grid_ecef(initial_position, radius, grid_spacing, buildings):
        ...
        #     if not any(building.contains(Point(pt[0], pt[1])) for building in buildings):
        #         safe_points.append((pt[0], pt[1], 0))

        # building_polygons = [Polygon(b["polygon"]) for b in buildings]
        # tree = STRtree(building_polygons)  # 构建空间索引
